'use client'
import React, { useMemo, useCallback, useState, useEffect, useDeferredValue, useTransition } from 'react'
...
const ClientsPage: React.FC = () => {
  ...
  }, [deferredSearch, search, setSearch, setPage])

  const currentPage = Number(page)
  const { data, isFetching, isLoading } = useClients(currentPage, deferredSearch ?? '')
  ...
  const showLoading = isLoading || (isFetching && !data?.clients?.length) || isPending || isTyping
    ...
    <div className="w-full">
      ...
      <div className="pt-8 relative">
        {showLoading ? (
          ...
          <>
            ...
            {isFetching && hasClients && (
              <div className="absolute inset-0 bg-white/50 flex items-center justify-center">
                <FiLoader className="text-dark-blue w-8 h-8 animate-spin" />
              </div>
            )}
          </>
        )}
      ...
      {!hasClients && !showLoading && (
        <div className="flex justify-center mt-4">
          <p className="text-dark-blue">No Results found for {deferredSearch}</p>
        </div>
      )}
