...
export async function updateSemesterProject(
  semesterId: string,
...
export async function getNextSemesterProjects(): Promise<{
  ...
  const fetchProjects = async (status: ProjectStatus) => {
    ...
    const projects: ProjectCardType[] = data.map(
      (semesterProject: SemesterProject, index: number) => {
        ...
        return {
          id: `item-${index}` as UniqueIdentifier, ////needs to contain 'item' for drag and drop logic
          projectInfo: {
  ...
  //TODO: need to order projects by their number!!
  return {
    data: {
