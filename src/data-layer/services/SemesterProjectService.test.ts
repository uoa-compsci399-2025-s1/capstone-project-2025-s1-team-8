...
describe('Semester service tests', () => {
  ...
  it('Should filter all semesterProjects by status', async () => {
    expect(
      (
        await semesterProjectService.filterSemesterProjectsByStatus(
  ...
  it('Should filter all semesterProjects by published status', async () => {
    expect(
      (
        await semesterProjectService.filterSemesterProjectsByPublished(
          false,
          semesterProjectListMock,
        )
      ).length,
  ...
  it('Should filter all semesterProjects by semester ID', async () => {
    expect(
      (
        await semesterProjectService.filterProjectsBySemester(
          '67ff38a56a35e1b6cf43a68j',
          semesterProjectListMock,
        )
      ).length,
    ).toEqual(1)
